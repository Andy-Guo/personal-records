...
__author__ = ""
__copyright__ = "Copyright(c) 2020, huawei"
__version__ = "1.0"
__email__ = ""
__status__ = "beta"
...
class MDTYPE2CONTEXTHEADER(Structure):
    _fields_ = [('metadata_class', c_ushort),
                ('md_cls_type', c_ubyte),
                ('reserved', c_ubyte, 1),
                ('length', c_ubyte, 7)]

    def __init__(self, metadata_class=0x0000, md_cls_type=0x00, length=0x00, data=bytes(), *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
        self.metadata_class = metadata_class
        self.md_cls_type = md_cls_type
        self.reserved = 0x00
        self.length = length
        self.data = data
        pad_len = 4 * length - len(data)
        if (pad_len > 0):
            for i in range(0,pad_len):
                self.data += bytes(0)
        if (pad_len < 0):
            mod = len(self.data) % 4
            self.length = ((len(data)/4) + 1) if (mod > 0) else (len(data) / 4)
        print(self.length)
    ...
    def build(self):
        v = pack('!H B B',
                    self.metadata_class,
                    self.md_cls_type,
                    (self.reserved & 0x80) | (self.length & 0x7F))
        if self.data:
            v = v + self.data
        return v
...
def decode_nsh_md2_contextheader(payload, offset, nsh_md2_hdr_value):
    "decode nsh md2 context data"
    ctx_hdr = payload[offset:offset + 4]
    _hdr_values = unpack('!H B B', ctx_hdr)
    nsh_md2_hdr_value.metadata_class = _hdr_values[0]
    nsh_md2_hdr_value.md_cls_type = _hdr_values[1]
    nsh_md2_hdr_value.length = _hdr_values[2]
    index = offset + 4;
    nsh_md2_hdr_value.data = payload[index: index + (nsh_md2_hdr_value.length) * 4]
...
def print_nsh_md2contextheader(nshmd2contextheader):
    print("NSH mdtype2[md_class,type,len, data] :", nshmd2contextheader.metadata_class, nshmd2contextheader.md_cls_type, nshmd2contextheader.length, nshmd2contextheader.data)
...
def main():
    ...
    if (args.do == "send"):
        ...
        mynshmd2contextheader = MDTYPE2CONTEXTHEADER()
        ...
        """ Set NSH mdtype1 context header """
        ...
        """ set NSH mdtype2 context header """
        data = bytes.encode("this is nsh various")
        mynshmd2contextheader.metadata_class = 0x0003
        mynshmd2contextheader.md_cls_type = 0x04
        mynshmd2contextheader.length = len(data)/4
        mynshmd2contextheader.data = data

        nsh_ctx_header = mynshcontextheader.build() if (mynshbaseheader.md_type == NSH_MD_TYPE1) else mynshmd2contextheader.build()
        ...
        if (args.type == "vxlan_gpe_nsh"):
            ...
            outerippack = build_udp_packet(args.outer_source_ip, args.outer_destination_ip, args.outer_source_udp_port, VXLAN_GPE_DPORT, myvxlanheader.build() + mynshbaseheader.build() + nsh_ctx_header + myethheader.build() + innerippack, False)
        elif (args.type == "eth_nsh"):
            #outerippack = mynshbaseheader.build() + mynshcontextheader.build() + myethheader.build() + innerippack
            outerippack = mynshbaseheader.build() + nsh_ctx_header + myethheader.build() + innerippack
        ...
        if (args.type == "eth_nsh"):
            ...
            decode_nsh_contextheader(ethpkt, offset + nshbase_length, mynshcontextheader) if mynshbaseheader.md_type == NSH_MD_TYPE1 \
            else decode_nsh_md2_contextheader(ethpkt, offset + nshbase_length, mynshmd2contextheader)
        ...
        while (args.number > 0):
            ...
            if (do_print):
                ...
                print_nsh_contextheader(mynshcontextheader) if mynshbaseheader.md_type == NSH_MD_TYPE1 \
                    else print_nsh_md2contextheader(mynshmd2contextheader)
            ...
            while True:
                ...
                if ((myethheader2.ethertype0 == 0x89) or (myethheader2.ethertype1 == 0x4f)):
                    ...
                    mynshmd2contextheader2 = MDTYPE2CONTEXTHEADER()
                    offset = eth_length
                    ...
                    decode_nsh_contextheader(packet, offset + nshbase_length, mynshcontextheader2) if (mynshbaseheader2.md_type == NSH_MD_TYPE1) \
                        else decode_nsh_md2_contextheader(packet, offset + nshbase_length, mynshmd2contextheader2)
                    ...
                    """ Print NSH context header """
                    print_nsh_contextheader(mynshcontextheader2) if  (mynshbaseheader2.md_type == NSH_MD_TYPE1) \
                        else print_nsh_md2contextheader(mynshmd2contextheader2)
                    break
                ...
                if (myudpheader2.udp_dport in vxlan_gpe_udp_ports):
                    ...
                    if (mynshbaseheader2.md_type == NSH_MD_TYPE1):
                        decode_nsh_contextheader(packet, offset, mynshcontextheader2)
                        offset += nshcontext_length
                    else:
                        decode_nsh_md2_contextheader(packet, offset, mynshmd2contextheader2)
                        offset += (mynshbaseheader2.length + 4)
    ...
    while True:
        ...
        if ((myethheader.ethertype0 == 0x89) or (myethheader.ethertype1 == 0x4f)):
            ...
            decode_nsh_contextheader(packet, offset + nshbase_length, mynshcontextheader) if (mynshbaseheader.md_type == NSH_MD_TYPE1) \
                else decode_nsh_md2_contextheader(packet, offset + nshbase_length, mynshmd2contextheader)
        ...
        if (myudpheader.udp_dport in vxlan_udp_ports):
            ...
            if ((myinsertedethheader.ethertype0 == 0x08) and (myinsertedethheader.ethertype1 == 0x00)):
                if ((args.do == "forward") and (args.interface is not None)):
                    ...
                    offset += nshbase_length
                    if (mynshbaseheader.md_type == NSH_MD_TYPE1):
                        decode_nsh_contextheader(vxlangpe_pkt, offset, mynshcontextheader)
                        offset += nshcontext_length
                    else:
                        decode_nsh_md2_contextheader(vxlangpe_pkt, offset, mynshmd2contextheader)
                        offset += (4 + mynshmd2contextheader.length)
                    ...
                    if (has_inserted_eth is True):
                        ...
                        ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, myvxlanheader.build() + myinsertedethheader.build() + mynshbaseheader.build() + nsh_ctx_header + vxlangpe_pkt[offset:], True)
                    else:
                        #ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, myvxlanheader.build() + mynshbaseheader.build() + nsh_ctx_header + vxlangpe_pkt[offset:], True)
            ...
            if (mynshbaseheader.md_type == NSH_MD_TYPE1):
                ...
                decode_nsh_md2_contextheader(packet, offset, mynshmd2contextheader)
                offset += (4 + mynshmd2contextheader.length)
                ...
                if args.forward_inner:
                    ...
                    if (myudpheader.udp_dport in vxlan_gpe_udp_ports):
                        ...
                        if (has_inserted_eth is True):
                            ...
                            ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, myvxlanheader.build() + myinsertedethheader.build() + mynshbaseheader.build() + nsh_ctx_header + packet[offset:], args.swap_ip)
                        else:
                            ...
                            ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, myvxlanheader.build() + mynshbaseheader.build() + nsh_ctx_header + packet[offset:], args.swap_ip)
                    else:
...
##python vxlan_tool.py -i eth0.2001 -d send --type vxlan_gpe_nsh  --outer-source-mac f4:4c:7f:30:d5:2a --outer-source-ip 70.202.7.197 --outer-destination-mac 38:bc:01:9b:d9:dc --outer-destination-ip 70.202.7.196 --inner-source-mac f4:4c:7f:30:d2:2c --inner-source-ip 10.10.4.36 --inner-destination-mac f4:4c:7f:40:a5:3f --inner-destination-ip 10.10.0.42 -n 1
